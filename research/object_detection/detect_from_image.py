...
def load_image_into_numpy_array(path):
    ...
    return np.reshape(np.array(image.getdata()), (im_height, im_width, 3)).astype(np.uint8)
...
def run_inference_for_single_image(model, image):
    ...
    if 'detection_masks' in output_dict:
        # Reframe  the box mask to the image size.
...
#  python .\detect_from_image.py -m ssd_mobilenet_v2_fpnlite_320x320_coco17_tpu-8\saved_model -l .\data\mscoco_label_map.pbtxt -i .\test_images
